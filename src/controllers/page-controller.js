...
export default class PageController {
  ...
  render(arrayFilms) {
    ...
    const filmsControllers = showFilms(arrayFilms.slice(0, this._lastFilmForShowing), this._allFilmsContainerElement);
  ...
  _renderExtraFilms() {
    ...
    for (let i = 0; i < EXTRA_FILMS_COUNT; i++) {
      const filmController = new FilmController(topRatedFilmsContainerElement);
      filmController.render(topRatedFilms[i]);
      this._showedFilmsControllers.push(filmController);
    }
    ...
    for (let i = 0; i < EXTRA_FILMS_COUNT; i++) {
      const filmController = new FilmController(mostCommentedFilmsContainerElement);
  ...
  _renderButtonShowMore() {
    ...
    this._buttonShowMoreComponent.setClickHandler(() => {
      ...
      const filmsControllers = showFilms(sortedFilms, this._allFilmsContainerElement);
      this._showedFilmsControllers = this._showedFilmsControllers.concat(filmsControllers);
